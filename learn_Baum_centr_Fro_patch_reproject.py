...
from LAF import denormalizeLAFs, LAFs2ell, extract_patches, abc2A
...
def create_loaders(load_random_triplets = False):
    ...
    train_loader = torch.utils.data.DataLoader(
            dset.HPatchesSeq('/datagrid/personal/mishkdmy/ucn-pytorch/dataset/', 'b',
...
def train(train_loader, model, optimizer, epoch, cuda = True):
    ...
    for batch_idx, data in pbar:
        ...
        if (len(LAFs1) == 0) or (len(LAFs2) == 0):
            optimizer.zero_grad()
            continue
        fro_dists, idxs_in1, idxs_in2, LAFs2_in_1 = get_GT_correspondence_indexes_Fro_and_center(LAFs1,LAFs2, H1to2,  dist_threshold = 2., 
                                                                             center_dist_th = 5.0,
        ...
        patch_dist = torch.mean((aff_norm_patches1[idxs_in1.data.long(),:,:,:] - aff_patches_from_LAFs2_in_1[idxs_in2.data.long(), :,:,:]) **2)
