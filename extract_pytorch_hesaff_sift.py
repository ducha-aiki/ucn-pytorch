...
def convert_LAF_to_this_stupid_Oxford_ellipse_format(img, LAFs):
    ...
    for i in range(len(LAFs)):
        ...
        LAF[0,2] *= float(img.shape[1])
        LAF[1,2] *= float(img.shape[0])
        scale = np.sqrt(LAF[0,0]*LAF[1,1]  - LAF[0,1]*LAF[1, 0])
        LAF[0:2,0:2] /=  scale;
...
class OrientationDetector(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(0, self.num_ang_bins):
            ang_bins.append(F.adaptive_avg_pool2d((bo0_big == i).float() * wo0_big, (1,1))) #+ (bo1_big == i).float() * wo1_big))
...
class HessianAffinePatchExtractor(nn.Module):
    ...
    def generate_scale_pyramid(self, x):
        ...
        while (curr[0,0,:,:].size(0) > minSize) and (curr[0,0,:,:].size(1) > minSize):
            for i in range(1,5):
                sigma = curSigma * np.sqrt(sigmaStep*sigmaStep - 1.0)
    ...
    def rotateLAFs(self, LAFs, angles):
        ...
        return torch.cat([temp_A, LAFs[:,:,2:]], dim = 2)
    def compose_affine_matrix(self, resp_map,  patch_size, mrSize, sigma = 1.0):
        rv = resp_map.view(-1)
        topk_val, idxs = torch.topk(rv, 
                                    k = max(1, min(int(self.num / 2), rv.size(0))));
        x = (idxs % resp_map.size(3)).float() /  float(resp_map.size(3))
        y = (idxs / resp_map.size(3)).float() /  float(resp_map.size(2))

        xy =  torch.cat([x.view(idxs.size(0),1,1),y.view(idxs.size(0),1,1)],dim = 1)
        min_size = min(float(resp_map.size(3)), float(resp_map.size(2)))
        A = sigma * mrSize * torch.eye(2).unsqueeze(0).expand(idxs.size(0),2,2).float() / min_size
        A = Variable(A)
        if self.use_cuda:
            A = A.cuda()
    ...
    def forward(self,x):
        ...
        for oct_idx in range(len(sigmas)):
            ...
            for level_idx in range(1,len(octave)-1):
                ...
                #print low.shape, cur.shape, high.shape
                #plt.figure()
                #plt.imshow(resp.data.cpu().numpy()[0,0,:,:])
                nms = self.NMS2d(cur) * (cur > low).float() * (cur > high).float()
                #plt.figure()
                #plt.imshow(deepcopy(nms.data.cpu().numpy()[0,0,:,:]))
                nms = self.zero_response_at_border(nms)
                #if ((nms.max() - nms.min()) <= 1e-12).data[0]:
                #    print 'skip this level'
                #    continue
                #plt.figure()
                #plt.imshow(nms.data.cpu().numpy()[0,0,:,:])
                aff_matrix,top_resp = self.compose_affine_matrix(nms,self.PS, self.mrSize, sigmas_oct[level_idx]);
...
HA = HessianAffinePatchExtractor( mrSize = 3.0, num_features = 3500, border = 5)
